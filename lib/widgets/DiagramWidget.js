    ...
    if (action instanceof SelectingAction && multiselectDrag) {
      ...
      action.selectionModels.forEach(model => {
        if (model.model instanceof NodeModel) {
          // We want to save the reference of the"new" position in a variable (same formula as before)
          let newPositionX =
            model.initialX +
            (event.pageX - this.state.action.mouseX) /
              (diagramModel.getZoomLevel() / 100);
          /*
            Math.round((newPositionX - model.initialX) / 27):
              Calculate how many 27px squares fit according to the movement
            * 27 + model.initialX:
              Substract or add "27px squares" to the initial position
          */
          model.model.x =
            Math.round((newPositionX - model.initialX) / 27) * 27 + model.initialX;
          // Same for Y
          let newPositionY =
            model.initialY +
            (event.pageY - this.state.action.mouseY) /
              (diagramModel.getZoomLevel() / 100);
          model.model.y =
            Math.round((newPositionY - model.initialY) / 27) * 27 + model.initialY;
        } else if (model.model instanceof PointModel) {
          // PointModel will continue with "fluent" formula
    ...
    if (element === null) {
      ...
      action.selectionModels.forEach(model => {
        ...
        if (model.model instanceof PointModel) {
          ...
          } else if (actionOutput.type !== 'point-created') {
            // We want to delete the created link
            model.model.link.remove();
            // And the PointModel as the whole operation will be cancelled
            model.model.remove();
            // Update the output
            actionOutput.type = 'link-cancelled';
            delete actionOutput.model;
          }
        }
      });
